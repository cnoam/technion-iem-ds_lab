...
def upload_file(number):
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                the_reply =  handle_file(filename,number)
            finally:
...
def handle_file(file_name, ex_number):
    ...
    completed_proc = None
    try:
        reference_output = "./data/ref_" + str(ex_number)
        #  https://medium.com/@mccode/understanding-how-uid-and-gid-work-in-docker-containers-c37a01d01cf
        completed_proc = subprocess.run( ['checker.sh',file_name, reference_output], check=False, capture_output = True, timeout = timeout)
        print(completed_proc.stdout)
        if completed_proc.returncode == 0:
            message = "well Done!"
        else:
            message = 'Your code failed. Please check the reported output\n\n' + str(completed_proc.stdout)
